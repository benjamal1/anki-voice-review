...
| A command fires repeatedly / a leftover word from the previous card acts on the next one | Streaming transcription re-emits the same recognised line every ~500ms until it slides out of the audio window, so an exact repeat within ~2.5s is suppressed automatically. Separately, whatever is still queued is discarded outright at every state change — a new card, the front-to-back flip — so a leftover recognition from before that point can never act on the state that follows it. The trace log's `duplicate-lines-suppressed` count and `drain` lines at session end show how much was caught. |
